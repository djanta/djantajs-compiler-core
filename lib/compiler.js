...
module.exports = class Compiler {
  ...
  _init(options) {
    ...
    (options || []).forEach((opt) => {
      ...
      glob.sync(opt.pattern || DEFAULT_FILTER, {cwd : opt.src, ignore : opt.excludes || DEFAULT_EXCLUDES}).forEach((file) => {
   ...
   */
  _introspect (project) {
    let self = this, reader = new Reader(self._registry), introspections = [], cwd = (project.src);

    self.logger ('Project Home: %s', cwd);
    glob.sync(project.pattern || DEFAULT_FILTER, {cwd : cwd, ignore: project.excludes || DEFAULT_EXCLUDES}).forEach((elected) => {
