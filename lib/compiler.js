...
module.exports = class Compiler {
  ...
  _init(options) {
    ...
    (options || []).forEach(opt => {
      ...
      glob.sync(opt.pattern || DEFAULT_FILTER, {cwd : opt.src, ignore : opt.excludes || DEFAULT_EXCLUDES})
          .forEach(file => {
  ...
  compile(options = {}, callback) {
    options.annotations = typeof options.annotations === 'string' ? [{
      ...
      : Array.isArray(options.annotations) ? options.annotations : !_.isNil(options.annotations) ? [options.annotations]
      : [];
    ...
    return !_.isNil(callback) && _.isFunction(callback) ? Promise.resolve().asCallback(callback)
      : Promise.resolve();
  }
};
