...
const { Registry } = require('conga-annotations');
const { Reader } = require('conga-annotations');
...
module.exports = class Compiler {
  ...
  _init(options) {
    ...
    (options || []).forEach((opt) => {
      glob.sync(opt.pattern || DEFAULT_FILTER, {cwd: opt.src
        , ignore: opt.excludes || DEFAULT_EXCLUDES}).forEach((file) => {

        let target = Path.resolve((opt.src), file);

        self.logger('Registeting annotation: <%s>', target);
  ...
  _introspect(project) {
    let self = this,
      reader = new Reader(self._registry),
      introspections = [], cwd = (project.src);

    glob.sync(project.pattern || DEFAULT_FILTER, {
      cwd: cwd, ignore: project.excludes || DEFAULT_EXCLUDES
    }).forEach((elected) => {
      let parsable = Path.resolve(cwd, elected);

      self.logger ('Introspecting: <%s>[%s]', project.src, elected);
      ...
      try {
        ...
        //console.log('%s', util.inspect(ex));
        self.error(util.inspect(ex));
      }
    });
  ...
  compile(options = {}, callback) {
    options.annotations = typeof options.annotations === 'string'? [{
      ...
      : (typeof options.annotations !== 'undefined' ? [options.annotations]: []);
    ...
    (this._handlers || []).forEach((handler) => {
      if (_.isFunction(handler)) {
        handler(argv);
      } else if (!_.isUndefined(handler.handle)) {
        handler.handle(argv);
      }
    });

    return !_.isUndefined(callback) && _.isFunction(callback) ? Promise.resolve().asCallback(callback)
      : Promise.resolve(ex);
  }
};
